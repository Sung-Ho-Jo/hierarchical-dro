...
class DRODataset(Dataset):
    ...
    # For CMNIST
    def rotate_images(self, angle, target_group=None):
        rotated_data = []
        for idx in range(len(self.dataset)):
            img, label, group = self.dataset[idx]
            if target_group is None or group == target_group:
                img = F.rotate(img, angle, fill=(0, 0, 0))
            rotated_data.append((img, label, group))
        self.dataset = rotated_data
